...
.directive('returnRate', function() {
  function link(scope, element, attrs) {
    ...
    // --------------------------------
    scope.render = function(data, senti, impact, entity) {
      ...
      getStock({ stock: symbol, startDate: "2014-01-01", endDate: "2015-01-01" }, 'historicaldata', function(err, data) {
        ...
        for (var i = 0; i < data.length; i++) {
          if (data[i].avgASenti >= senti && data[i].avgImpSc >= impact) {
            // buy with capital
            var sharePrice = price[i].Close
            var numShare = Math.floor(capital/sharePrice)
            invested = Math.floor(capital/sharePrice) * sharePrice
            if (i === 0) {
              capital -= invested
            }
            else {
              numShare += strategyLog[i-1].numShare
              invested += strategyLog[i-1].invested
              capital -= Math.floor(capital/sharePrice) * sharePrice
            }
            // push into strategyLog array
            insert(strategyLog, data[i].date, capital, invested, numShare, sharePrice)
            console.log("BUY -- " + "date:" + strategyLog.date + ", price:" + strategyLog.sharePrice + ", numShare:" + strategyLog.numShare + ", invested:" + strategyLog.invested + ", capital:" + strategyLog.capital)
          }
          else {
            // sell stock
            var sharePrice = price[i].Close
            ...
            invested = 0
            if (i > 0)
              capital += strategyLog[i-1].numShare * sharePrice
            // push into strategyLog array
            insert(strategyLog, data[i].date, capital, invested, numShare, sharePrice)
            console.log("SELL -- " + "date:" + strategyLog.date + ", price:" + strategyLog.sharePrice + ", numShare:" + strategyLog.numShare + ", invested:" + strategyLog.invested + ", capital:" + strategyLog.capital)
          }
        }
      })
    ...
    // -----------------------------------------------------------
    scope.$watchGroup(['data', 'senti', 'impact', 'entity'], function(newVals, oldVals) {
      scope.render(newVals[0], newVals[1], newVals[2], newVals[3])
    }, true)
  }
  return {
      link: link,
      restrict: 'E',
      scope: { data: '=', senti: '=', impact: '=', entity: '='}
  }
})